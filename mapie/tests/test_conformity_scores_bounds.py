...
conf_scores_residual_norm_list = np.array([0.2, 0.0, 0.11111111, 0.09090909, 0.0, 0.2])
random_state = 42
...
def test_check_wrong_regression_score(score: Any) -> None:
    """
...
def test_absolute_conformity_score_get_conformity_scores(
    ...
    conf_scores = abs_conf_score.get_conformity_scores(y_toy, y_pred, X=X_toy)
...
@pytest.mark.parametrize("y_pred", [np.array(y_pred_list), y_pred_list])
@pytest.mark.parametrize("conf_scores", [np.array(conf_scores_list), conf_scores_list])
def test_absolute_conformity_score_get_estimation_distribution(
    ...
    abs_conf_score = AbsoluteConformityScore()
    y_obs = abs_conf_score.get_estimation_distribution(y_pred, conf_scores, X=X_toy)
...
def test_gamma_conformity_score_get_conformity_scores(
    ...
    conf_scores = gamma_conf_score.get_conformity_scores(y_toy, y_pred, X=X_toy)
...
def test_gamma_conformity_score_get_estimation_distribution(
    ...
    gamma_conf_score = GammaConformityScore()
    y_obs = gamma_conf_score.get_estimation_distribution(y_pred, conf_scores, X=X_toy)
    np.testing.assert_allclose(y_obs, y_toy)
...
def test_gamma_conformity_score_check_observed_value(
    ...
    with pytest.raises(ValueError):
        gamma_conf_score.get_signed_conformity_scores(y_toy, y_pred, X=[])
...
def test_gamma_conformity_score_check_predicted_value(
    ...
    gamma_conf_score = GammaConformityScore()
    with pytest.raises(
        ValueError, match=r".*At least one of the predicted target is negative.*"
    ):
        gamma_conf_score.get_signed_conformity_scores(y_toy, y_pred, X=X_toy)
    with pytest.raises(
        ValueError, match=r".*At least one of the predicted target is negative.*"
    ):
        gamma_conf_score.get_estimation_distribution(y_pred, conf_scores, X=X_toy)
...
def test_residual_normalised_prefit_conformity_score_get_conformity_scores(
    ...
    residual_norm_conf_score = ResidualNormalisedScore(
        residual_estimator=residual_estimator, prefit=True, random_state=random_state
    )
    conf_scores = residual_norm_conf_score.get_conformity_scores(y_toy, y_pred, X=X_toy)
...
def test_residual_normalised_conformity_score_get_conformity_scores(
    ...
    conf_scores = residual_norm_score.get_conformity_scores(y_toy, y_pred, X=X_toy)
...
def test_residual_normalised_score_prefit_with_notfitted_estim() -> None:
    ...
    with pytest.raises(ValueError):
        residual_norm_conf_score.get_conformity_scores(y_toy, y_pred_list, X=X_toy)
...
def test_residual_normalised_score_with_default_params() -> None:
    ...
    conf_scores = residual_norm_score.get_conformity_scores(y_toy, y_pred_list, X=X_toy)
    residual_norm_score.get_estimation_distribution(y_toy, conf_scores, X=X_toy)
...
def test_cross_residual_normalised() -> None:
    ...
    """
    with pytest.raises(ValueError):
        _MapieRegressor(conformity_score=ResidualNormalisedScore()).fit(X_toy, y_toy)
...
def test_residual_normalised_score_pipe() -> None:
    ...
    mapie_reg.fit(np.concatenate((X_toy, X_toy)), np.concatenate((y_toy, y_toy)))
...
def test_residual_normalised_additional_parameters() -> None:
    ...
    """
    residual_normalised_conf_score = ResidualNormalisedScore(
        residual_estimator=LinearRegression(), split_size=0.2, random_state=random_state
    )
    # Test for get_conformity_scores
    # 1) Test that no error is raised
    residual_normalised_conf_score.get_conformity_scores(y_toy, y_pred_list, X=X_toy)
    # 2) Test that an error is raised when X is not provided
    with pytest.raises(ValueError, match=r"Additional parameters must be provided*"):
        residual_normalised_conf_score.get_conformity_scores(y_toy, y_pred_list)
...
def test_intervals_shape_with_every_score(
    score: BaseRegressionScore, alpha: NDArray
) -> None:
    estim = LinearRegression().fit(X_toy, y_toy)
    mapie_reg = _MapieRegressor(
